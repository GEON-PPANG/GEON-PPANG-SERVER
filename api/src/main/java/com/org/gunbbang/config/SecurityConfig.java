...
public class SecurityConfig {
    ...
    public SecurityFilterChain filterChain(HttpSecurity http) throws Exception {
        http
                ...
                .antMatchers("/","/css/**","/images/**","/js/**","/favicon.ico","/h2-console/**", "/health", "/profile").permitAll()
                .antMatchers("/member/signup", "/validate/nickname", "/validate/email").permitAll() // 회원가입, 닉네임 중복체크, 이메일 중복체크는은모든 리소스에 접근 가능
                .antMatchers("/health", "/profile", "/actuator/health").permitAll()
