...
/**
 * JWT 인증 필터
 * "/login" 이외의 uri 요청이 왔을 때 해당 요청을 처리함 (로그인 요청은 엑세서 or 리프레시 토큰 받기 전이니까)
 * refreshToken은 토큰 재발급 요청 시에만 오고 일반적으로는 엑세스 토큰만 와서 인증 요청함
 *
 * [일반적인 인증 요청] 1. refreshToken이 없고 accessToken이 유효한 경우 -> 인증 성공
 * [잘못된 인증 요청]  2. refreshToken이 없고 accessToken이 없거나 유효하지 않은 경우 -> 잘못된 인증 요청, 인증 실패, 403 반환
 * [토큰 재발급 요청]  3. refreshToken이 있는 경우 -> db에 refreshToken 조회해서 일치하면 둘 다 재발급, 조회 못하면 에러처리
 *  TODO: 3번 관련해서 해당 요청에 accessToken이 아직 만료 안된거라면 400에러 떨궈야되는거 아닌가?
 */
...
public class JwtAuthenticationProcessingFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        /**
         * 헤더에 유효한 refreshToken이 담겨져서 요청된 경우 -> 토큰 재발급 요청
         * refresh와 access 둘 다 재발급해서 반환
         */
        ...
        /**
         * refreshToken가 null인 경우 -> 일반적인 인증 요청
         * 이지만 accesssToken이 유효하지 않을 경우 403을 내려줘야함
         * accessToken 유효성 검사
         */
        if (refreshToken == null) {
    ...
    private void checkRefreshTokenAndReIssueAccessToken(HttpServletResponse response, String refreshToken) {
        memberRepository.findByRefreshToken(refreshToken)
                .ifPresent(member -> {
                    ...
                    String reIssuedAccessToken = jwtService.createAccessToken(member.getEmail(), member.getMemberId());
    ...
    /**
     * accessToken 추출하고 유효한지 검증
     * 유효하면 claim에서 memberId 추출하고 해당 memberId 조회된 유저 객체 반환
