 ...
 * 로그인 요청이 왔을 때 해당 유저가 유효한 유저인지 인증 처리함
 */
public class CustomJsonUsernamePasswordAuthenticationFilter extends AbstractAuthenticationProcessingFilter {
    ...
    public CustomJsonUsernamePasswordAuthenticationFilter (ObjectMapper objectMapper) {
        ...
        // 생성자 매개변수로 DEFAULT_LOGIN_PATH_REQUEST_MATCHER(authentication이 필요한지 필요하지 않은지 결정하기 위해 사용되는 RequestMatcher)를 넘겨줌
     ...
     * UsernamePasswordAuthenticationFilter와 동일하게 AbstractAuthenticationProcessingFilter의 attemptAuthentication 오버라이딩해서 사용
    ...
    public Authentication attemptAuthentication(HttpServletRequest request, HttpServletResponse response) throws AuthenticationException, IOException {
        ...
        // authenticate 설명: 전달받은 Authentication 인증 객체를 인증 처리하려는 시도. 성공할 경우 완전하게 채워진 Authentication 객체를 반환
